...
public class MethodMetadataDecoderImpl implements MethodMetadataDecoder {
    ...
    public Field[] parseFields(DynamicHub declaringType, byte[] encoding, boolean publicOnly) {
        ...
        return decodeArray(reader, Field.class, (i) -> decodeField(reader, codeInfo, DynamicHub.toClass(declaringType), publicOnly));
    }
    ...
    public Method[] parseMethods(DynamicHub declaringType, byte[] encoding, boolean publicOnly) {
        UnsafeArrayTypeReader reader = UnsafeArrayTypeReader.create(encoding, 0, ByteArrayReader.supportsUnalignedMemoryAccess());
        CodeInfo codeInfo = CodeInfoTable.getImageCodeInfo();
        return decodeArray(reader, Method.class, (i) -> decodeMethod(reader, codeInfo, DynamicHub.toClass(declaringType), publicOnly));
    }
    ...
    public Constructor<?>[] parseConstructors(DynamicHub declaringType, byte[] encoding, boolean publicOnly) {
        UnsafeArrayTypeReader reader = UnsafeArrayTypeReader.create(encoding, 0, ByteArrayReader.supportsUnalignedMemoryAccess());
        CodeInfo codeInfo = CodeInfoTable.getImageCodeInfo();
        return decodeArray(reader, Constructor.class, (i) -> decodeConstructor(reader, codeInfo, DynamicHub.toClass(declaringType), publicOnly));
    }
    ...
    private static Method decodeMethod(UnsafeArrayTypeReader buf, CodeInfo info, Class<?> declaringClass, boolean publicOnly) {
        return (Method) decodeExecutable(buf, info, declaringClass, publicOnly, true);
    }
    ...
    private static Constructor<?> decodeConstructor(UnsafeArrayTypeReader buf, CodeInfo info, Class<?> declaringClass, boolean publicOnly) {
        return (Constructor<?>) decodeExecutable(buf, info, declaringClass, publicOnly, false);
    }

    private static Executable decodeExecutable(UnsafeArrayTypeReader buf, CodeInfo info, Class<?> declaringClass, boolean publicOnly, boolean isMethod) {
        int modifiers = buf.getUVInt();
        boolean inHeap = (modifiers & IN_HEAP_FLAG_MASK) != 0;
        if (inHeap) {
        ...
        executable.rawParameters = reflectParameters.length > 0 ? reflectParameters : null;
